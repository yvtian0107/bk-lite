...
class TestExplainResource:
    ...
    def test_deployment_api_version(self):
        out = json.loads(c.explain_kubernetes_resource.invoke({"resource_type": "deployment"}))
        assert out["api_version"] == "apps/v1"

    def test_unknown_resource(self):
        out = json.loads(c.explain_kubernetes_resource.invoke({"resource_type": "widget"}))
        assert "error" in out
...
class TestTroubleshootingGuide:
    def test_known_keyword(self):
        out = json.loads(c.kubernetes_troubleshooting_guide.invoke({"keyword": "POD", "namespace": "prod"}))
        assert out["title"] == "Pod故障排查指导"
        assert out["namespace_context"] == "prod"
        assert len(out["steps"]) == 5

    def test_network_keyword(self):
        out = json.loads(c.kubernetes_troubleshooting_guide.invoke({"keyword": "network"}))
        assert "网络" in out["title"]

    def test_unknown_keyword(self):
        out = json.loads(c.kubernetes_troubleshooting_guide.invoke({"keyword": "quantum"}))
        assert "error" in out
...
def apis():
    ...
    with patch.object(c, "prepare_context", return_value=None), patch.object(c.client, "CoreV1Api", return_value=core), patch.object(
        c.client, "AppsV1Api", return_value=apps
    ):
        yield core, apps
...
class TestVerifyConnection:
    def test_success_via_call_api(self, apis):
        core, _ = apis
        core.api_client.call_api.return_value = ({"gitVersion": "v1.28.0", "platform": "linux/amd64"},)
    ...
    def test_fallback_to_version_api(self, apis):
        ...
        with patch.object(c.client, "VersionApi") as VApi:
            VApi.return_value.get_code.return_value = SimpleNamespace(git_version="v1.27.3", platform="linux/arm64")
            core.list_namespace.return_value = SimpleNamespace(items=[])
            out = json.loads(c.verify_kubernetes_connection.invoke({"config": {}}))
...
class TestListApiResources:
    def test_aggregates_core_resources(self, apis):
        core, _ = apis
        res = SimpleNamespace(name="pods", short_names=["po"], kind="Pod", namespaced=True, verbs=["get", "list"])
        sub = SimpleNamespace(name="pods/log", short_names=None, kind="Pod", namespaced=True, verbs=None)
        core.get_api_resources.return_value = SimpleNamespace(resources=[res, sub])
        with patch.object(c.client, "ApiClient"), patch.object(c.client, "ApisApi") as ApisApi, patch.object(
            c.client, "CoreV1Api", return_value=core
        ):
            ApisApi.return_value.get_api_versions.return_value = SimpleNamespace(groups=[])
...
class TestDescribeResource:
    def test_pod_requires_namespace(self, apis):
        out = json.loads(c.describe_kubernetes_resource.invoke({"resource_type": "pod", "resource_name": "p", "config": {}}))
    ...
    def test_node_describe(self, apis):
        ...
        with patch.object(c.client, "ApiClient") as AC:
            ...
            out = json.loads(c.describe_kubernetes_resource.invoke({"resource_type": "node", "resource_name": "n1", "config": {}}))
        assert out["name"] == "n1"
        assert out["resource_type"] == "node"
        assert out["labels"] == {"a": "b"}

    def test_unsupported_type(self, apis):
        out = json.loads(c.describe_kubernetes_resource.invoke({"resource_type": "widget", "resource_name": "x", "config": {}}))
        assert "暂不支持" in out["error"]
        assert "pod" in out["supported_types"]
        assert "statefulset" in out["supported_types"]

    def test_statefulset_describe(self, apis):
        _, apps = apis
        with patch.object(c.client, "ApiClient") as AC:
            AC.return_value.sanitize_for_serialization.return_value = {
                "metadata": {"name": "nacos", "namespace": "nacos", "labels": {"app": "nacos"}},
                "spec": {"replicas": 3},
                "status": {"readyReplicas": 2},
            }
            out = json.loads(
                c.describe_kubernetes_resource.invoke({"resource_type": "statefulset", "resource_name": "nacos", "namespace": "nacos", "config": {}})
            )
        assert out["name"] == "nacos"
        assert out["resource_type"] == "statefulset"
        apps.read_namespaced_stateful_set.assert_called_once_with("nacos", "nacos")

    def test_statefulset_requires_namespace(self, apis):
        out = json.loads(c.describe_kubernetes_resource.invoke({"resource_type": "sts", "resource_name": "nacos", "config": {}}))
        assert "需要指定namespace" in out["error"]

    def test_not_found_404(self, apis):
        core, _ = apis
        core.read_node.side_effect = ApiException(status=404)
        out = json.loads(c.describe_kubernetes_resource.invoke({"resource_type": "node", "resource_name": "ghost", "config": {}}))
    ...
    def test_other_api_error(self, apis):
        ...
        out = json.loads(c.describe_kubernetes_resource.invoke({"resource_type": "namespace", "resource_name": "x", "config": {}}))
...
class TestGetContexts:
    def test_multi_instance(self):
        ...
        with patch("apps.opspilot.metis.llm.tools.kubernetes.connection." "get_kubernetes_instances_from_configurable", return_value=instances):
            out = json.loads(c.get_kubernetes_contexts.func(config={"configurable": {"kubernetes_instances": [1, 2]}}))
    ...
    def test_local_kubeconfig_error(self):
        with patch("kubernetes.config.list_kube_config_contexts", side_effect=Exception("no config")):
            out = json.loads(c.get_kubernetes_contexts.invoke({"config": {}}))
