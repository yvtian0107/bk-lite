...
class PrometheusAdapter(AlertSourceAdapter):
    ...
    def fetch_alerts(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = requests.get(url, timeout=self.config.get("timeout", 10), verify=self.config.get("verify_ssl", True))
    ...
    def normalize_payload(self, payload: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for alert in alerts:
            ...
            resource_name = labels.get("instance") or labels.get("pod") or labels.get("node") or labels.get("service") or labels.get("job")
...
""".strip()
    ...
    def _map_prometheus_status(self, status: str) -> str:
        status_map = {"firing": "created", "resolved": "recovery"}
