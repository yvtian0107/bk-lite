...
from apps.operation_analysis.services.datasource_preview.excel import MAX_EXCEL_BYTES, _normalize_dataframe_rows
    ...
    try:
        ...
        with ZipFile(file_obj) as archive:
            ...
            if len(infos) > MAX_XLSX_ARCHIVE_ENTRIES:
                raise ConnectorError(oa_message("messages.excel_archive_entries", "Excel 压缩包条目过多"), code="excel_archive_too_large", status_code=400)
            decoded_size = sum(max(0, info.file_size) for info in infos)
            if decoded_size > MAX_XLSX_UNCOMPRESSED_BYTES:
                raise ConnectorError(
                    oa_message("messages.excel_unzipped_too_large", "Excel 解压后体积过大"), code="excel_archive_too_large", status_code=400
                )
    except ConnectorError:
        raise
    except (BadZipFile, OSError, ValueError) as exc:
        raise ConnectorError(oa_message("messages.excel_format_invalid", "Excel 文件格式无效"), code="excel_file_type_invalid", status_code=400) from exc
    finally:
        if hasattr(file_obj, "seek"):
            file_obj.seek(0)
        ...
        for row in rows:
            if any(value is not None for value in row):
                ...
                if data_rows > MAX_MATERIALIZE_ROWS:
                    raise ConnectorError(
                        oa_message("messages.excel_row_limit", "Excel 行数超过 {limit}，无法处理", limit=MAX_MATERIALIZE_ROWS),
...
def read_excel_rows_for_materialize(file_obj, sheet_name: str | None = None) -> list[dict[str, Any]]:
    ...
    if dataframe.empty:
        raise ConnectorError(oa_message("messages.excel_process_empty", "Excel 没有可处理的数据"), code="excel_empty", status_code=400)
