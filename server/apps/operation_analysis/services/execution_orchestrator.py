...
from apps.operation_analysis.services.dashboard_report_renderer import DashboardRenderContractError
...
from apps.operation_analysis.services.execution_timeout_checker import ExecutionTimeoutChecker
...
from apps.operation_analysis.services.retry_classifier import MAX_ATTEMPTS, RetryClassifier
from apps.operation_analysis.services.retry_types import AttemptResult, ClassifierDecisionKind, ResumeClass
...
def _as_attempt_result(value) -> AttemptResult:
    if isinstance(value, AttemptResult):
        return value
    if value is ExecutionStepResult.COMPLETED or value == (ExecutionStepResult.COMPLETED):
    ...
    def execute(
        ...
        if users.count() != 1:
            return AttemptResult(
                ...
                error_message=oa_message("messages.execution_creator_denied", "Execution 创建者无权查看仪表盘"),
            )
        ...
        from apps.operation_analysis.services.canvas_report.permissions import can_view_canvas, canvas_resource_exists
        ...
        if live_missing:
            if execution.source_canvas_deleted_during_execution and cls._has_render_snapshot(execution):
                return AttemptResult(ok=True)
            return AttemptResult(
                ok=False,
                failure_stage=cls.failure_stage,
    ...
    def execute(
        ...
        try:
            ...
            return AttemptResult(
                ...
                error_message=oa_message("messages.execution_snapshot_missing", "Execution Snapshot 不存在"),
            )

        if execution.source_canvas_deleted_during_execution and execution.resource_id is None and execution.dashboard_id is None:
            is_valid = (
            ...
            exec_resource_id = execution.resource_id if execution.resource_id is not None else execution.dashboard_id
            snap_resource_id = snapshot.resource_id if snapshot.resource_id is not None else snapshot.dashboard_id
            is_valid = (
                exec_resource_id is not None
                and execution.subscription_id is not None
                and snap_resource_id == exec_resource_id
                and (not snapshot.resource_type or not execution.resource_type or snapshot.resource_type == execution.resource_type)
                and snapshot.creator_id == execution.creator
                and snapshot.creator_domain == execution.creator_domain
                and snapshot.subscription_id == execution.subscription_id
                and isinstance(snapshot.filter_values, dict)
        ...
        if not is_valid:
            return AttemptResult(
                ...
                error_message=oa_message("messages.execution_snapshot_invalid", "Execution Snapshot 内容无效"),
            )
        return snapshot
    ...
    def execute(
        ...
        try:
            ...
            return AttemptResult(
                ...
                error_message=str(exc) or oa_message("messages.render_snapshot_create_failed", "Render Snapshot 创建失败"),
            ...
            if transient:
                return AttemptResult(
                    ...
                    error_message=oa_message("messages.render_snapshot_create_failed", "Render Snapshot 创建失败"),
                    side_effect=type(exc).__name__,
                )
            # 未知异常：不 retry
            return AttemptResult(
...
class RenderStep:
    ...
    def execute(
        ...
        try:
            ...
            logger.warning(
                "Dashboard Render Contract 失败: " "execution_id=%s widget_id=%s stage=%s code=%s",
                execution.id,
                exc.widget_id,
            ...
            return AttemptResult(
                ok=False,
                failure_stage=getattr(exc, "failure_stage", None) or cls.failure_stage,
                error_code=exc.error_code or "render_contract_business_failed",
            ...
            persisted = getattr(exc, "persisted_message", None)
            if callable(persisted):
                safe_message = persisted()
            else:
                safe_message = getattr(exc, "safe_message", None)
                if not isinstance(safe_message, str) or not safe_message:
                    safe_message = oa_message("messages.pdf_generate_failed", "报告 PDF 生成失败")
...
class DeliveryStep:
    ...
    def execute(
        ...
        try:
            ...
            failure_stage = "render" if error_code == "pdf_generate_failed" else cls.failure_stage
            return AttemptResult(
                ok=False,
            ...
            return AttemptResult(
                ...
                failure_stage=cls.failure_stage,
                error_code="",
                error_message=oa_message("messages.email_delivery_failed", "邮件投递失败"),
                side_effect=type(exc).__name__,
            )
...
class ExecutionOrchestrator:
    ...
    def execute(cls, execution_id: int) -> DashboardReportExecution:
        ...
        if execution.status != DashboardReportExecution.Status.RUNNING:
            raise ValidationError({"status": oa_message("messages.execution_claim_worker", "Execution 必须先由 Worker 成功领取")})
        ...
        while True:
            ...
            if resource.delivery_outcome == "delivered":
                ...
                if execution.status == DashboardReportExecution.Status.SUCCEEDED:
                    return execution
            ...
            if result.side_effect:
                logger.info(
                    "Attempt 副作用(仅日志): execution_id=%s " "attempt=%s side_effect=%s",
                    execution.id,
                    attempt_no,
                    result.side_effect,
                )
            ...
            if decision.kind == ClassifierDecisionKind.TERMINAL_UNKNOWN:
                ...
                execution.refresh_from_db()
                if execution.status == DashboardReportExecution.Status.UNKNOWN:
                    return execution
                return DashboardReportExecutionService.transition(
                    execution,
            ...
            if ExecutionTimeoutChecker.is_past_deadline(execution):
                ...
                return execution
            logger.info(
                "Execution 进入下一 attempt: execution_id=%s " "attempt=%s resume_class=%s",
                execution.id,
                attempt_no,
                resume_class,
            )
        ...
        *,
        ...
        try:
            ...
            return AttemptResult(
                ...
                error_message=oa_message("messages.resume_class_unknown", "未知 resume_class: {resume_class}", resume_class=resume_class),
            )
    ...
    def _run_full(
        ...
        result = _as_attempt_result(RenderStep.execute(execution, snapshot, render_snapshot))
        if not result.ok:
            return result

        return _as_attempt_result(DeliveryStep.execute(execution, snapshot))
    ...
    def _run_from_render(
        ...
        try:
            ...
            return AttemptResult(
                ...
                failure_stage="render_snapshot",
                error_code="render_snapshot_create_transient",
                error_message=oa_message("messages.render_snapshot_missing", "Render Snapshot 不存在"),
            )
        result = _as_attempt_result(RenderStep.execute(execution, snapshot, render_snapshot))
        if not result.ok:
            return result
    ...
    def _run_from_delivery(
        ...
        if resource.artifact != "valid":
            return AttemptResult(
                ...
                error_message=oa_message("messages.delivery_artifact_unavailable", "Delivery resume 时 Artifact 不可用"),
            )
        return _as_attempt_result(DeliveryStep.execute(execution, snapshot))
